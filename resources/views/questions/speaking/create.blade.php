...
@section('contents')
    @php
        $isUpdate = isset($speakingQuestion);
    @endphp
    <div class="mt-4">
        <div class="row g-4">
                    ...
                    <div class="card shadow-none border my-4" data-component-card="data-component-card">
                            ...
                            <div class="row g-3 justify-content-between align-items-center">
                                <div class="col-12 col-md">
                                    <h4 class="text-body mb-0">
                                        @if ($isUpdate)
                                            Update Speaking Question For Part {{ $part->title }} ( {{ $part->skill->type->label() }})
                                        @else
                                            Create Speaking Question For Part {{ $part->title }} ( {{ $part->skill->type->label() }})
                                        @endif
                                    </h4>
                                </div>
                            </div>
                        ...
                        <div class="card-body p-0">
                            <div class="p-4 code-to-copy">
                                @if ($isUpdate)
                                <form action="{{ route('admin.parts.speaking-questions.update', [$part->id, $speakingQuestion->id]) }}" method="POST">
                                    @method('PUT')
                                @else
                                ...
                                @endif
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <textarea id="editor" class="form-control {{ $errors->has('content') ? 'is-invalid' : '' }}"
                                                  name="content">{{ old('content', $isUpdate ? $speakingQuestion->content : '') }}</textarea>
